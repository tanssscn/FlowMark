...
import { FolderOpened, InfoFilled, Remove, FolderAdd, Document, DocumentAdd, Delete, Edit, Refresh } from '@element-plus/icons-vue'
...
const { remove, removeFromTree, refresh } = useFile()
...
const menuItems = computed((): MenuItem[] => [
  ...
  },
  {
    ...
    icon: DocumentAdd,
    visible: (data: FileEntry | null) =>
      !(data?.storageLocation === 'local' && isBrowser),
    action: async (data: FileEntry) => {
      if (data.isDir) {
        emit('newFile', props.contextMenu.node, data)
      } else {
        emit('newFile', props.contextMenu.node.parent, props.contextMenu.node.parent.data)
      }
    }
  },
  {
    ...
    action: async (data: FileEntry) => {
      if (data.isDir) {
        ...
        emit('newFolder', props.contextMenu.node.parent, props.contextMenu.node.parent.data)
      }
    }
  },
  {
