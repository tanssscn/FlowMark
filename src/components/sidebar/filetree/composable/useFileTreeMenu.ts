...
import { getDirname, getExtname, getFilename, getJoin, isValidFilePath } from '@/utils/pathUtil';
...
export function fileTree(renameState: RenameState, contextMenu: ContextMenuState,
  ...
  const { move, rename, refresh } = useFile()
  ...
  const create = async (node: any, data: FileEntry, isDirectory: boolean) => {
    ...
    if (!newName) return
    let newPath = getJoin(data.path, normalizeString(newName, false))
    ...
    if (!isDirectory) {
      const ext = getExtname(newPath)
      if (!ext) {
        newPath += `.${settingsStore.state.file.defaultFileExtension}`
      }
    }
    // 创建新文件/文件夹
    const newEntry = {
