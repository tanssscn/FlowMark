...
import { FolderOpened, Plus, Refresh, Document, Folder } from '@element-plus/icons-vue'
...
const createNewFile = () => {
  ...
  const currentNode = treeRef.value?.getNode(currentData)
  if (currentData?.isDir) {
    create(currentNode, currentData as FileEntry, false)
  } else {
    create(currentNode?.parent, currentNode?.parent?.data as FileEntry, false)
  }
}
...
const createNewFolder = () => {
  ...
  const currentNode = treeRef.value?.getNode(currentData)
  if (currentData?.isDir) {
    create(currentNode, currentData as FileEntry, true)
  } else {
    create(currentNode?.parent, currentNode?.parent?.data as FileEntry, true)
  }
}

const refreshTree = () => {
  const rootEntries = fileStore.fileTree
  ...
  <div>
    ...
    <el-scrollbar class="h-full pr-3!">
      <el-tree ref="treeRef" :data="fileStore.fileTree" :props="treeProps" draggable :allow-drag="allowDrag"
        ...
        :current-node-key="tabStore.state?.filePath" check-on-click-node class="h-full">
