  ...
  <div v-show="tabStore.activeSession" class="h-full flex flex-col overflow-hidden">
    ...
    <el-scrollbar v-else class="flex-1 !p-2">
      <el-timeline :reverse="true">
        <el-timeline-item v-for="version in versions" :key="version.id"
          :timestamp="formatDate(version.createdAt, 'YYYY-MM-DD HH:mm:ss')" placement="top">
...
import { Plus, Document, DeleteFilled } from '@element-plus/icons-vue'
...
import type { VersionContextMenuState } from './composable/useVersion.ts'
import { useVersion } from './composable/useVersion.ts'
...
const buttons = computed(() => [
  ...
  {
    label: t('version.clearAll'),
    icon: DeleteFilled,
    click: deleteAllVersion,
  }
])
const deleteAllVersion = () => {
  const info = fileInfo.value
  if (!info) return
  dialogService.confirm({
    ...
    versionService.deleteVersions(info.path)
      .then(() => {
        versions.value = []
      })
  })
