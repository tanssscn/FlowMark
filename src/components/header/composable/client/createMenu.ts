...
import { type MenuConfig, useMenuConfig } from '@/components/header/composable/menuConfig';
...
export function menuWatch(menu: Menu | undefined): () => void {
  ...
  const unwatchRecentFiles = watch(recentStore.state, async () => {
    const menuItems = await Promise.all(
      recentStore.state.map(async file => {
