    ...
    <el-form label-position="top" class="space-y-4">
      ...
      <div class="space-y-4">
        <h4 class="mb-2 text-base">
          {{ t('settings.markdown.extensions.label') }}
        </h4>
        <el-form-item>
          <div class="flex items-center">
            <el-switch v-model="settings.markdown.extensions.enableMermaid" />
            <span class="ml-2 text-sm">
              {{ t('settings.markdown.extensions.enableMermaid') }}
            </span>
        ...
        <el-form-item>
          <div class="flex items-center">
            <el-switch v-model="settings.markdown.extensions.enableEmoji" />
            <span class="ml-2 text-sm">
              {{ t('settings.markdown.extensions.enableEmoji') }}
            </span>
          </div>
        </el-form-item>
      </div>
      ...
      <div class="space-y-4">
        ...
        <el-form-item>
          <el-select v-model="settings.markdown.tocDepth" multiple class="w-full max-w-xs">
            <el-option v-for="depth in [1, 2, 3, 4, 5, 6]" :key="depth" :label="`H${depth}`" :value="depth" />
          </el-select>
        </el-form-item>
      </div>
