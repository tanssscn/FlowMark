...
export function useFile() {
  ...
  async function openFile() {
    localFileService.openLocalFile({
      title: t('menu.FileMenu.openFile'),
      filters: [
  ...
  async function addWebDavFileTree(fileInfo: Pick<FileEntry, 'path' | 'storageLocation' | 'isDir' | 'username' | 'rootPath'>) {
    fileInfo.rootPath = fileInfo.path
    const _fileInfo = fileStore.get(fileInfo.path)
    if (_fileInfo) {
      return
    }
    await loadWebdavTreeWithRetry(fileInfo)
  }

  async function loadWebdavTreeWithRetry(fileInfo: Pick<FileEntry, 'path' | 'storageLocation' | 'isDir' | 'username' | 'rootPath'>) {
    try {
      console.log('Loaded directory:', fileInfo.path); // 添加日志
      const stat = await fileService.getStat(fileInfo)
      if (stat.isDir) {
        const fileEntry = await fileService.readDirectory(fileInfo)
        loadFiletreeAndRecent(fileEntry)
      } else {
      ...
      const result = await dialogService.confirm({
        title: t('notify.errors.webdavLoadFailed'),
        message: t('notify.errors.webdavLoadFailedMessage'),
      ...
      if (result) {
        ...
        removeFromTree({ path: fileInfo.path, children: [] })
      }
    }
  }
  ...
  function clearAllRecent() {
    dialogService.confirm({
      title: t('menu.FileMenu.clearRecentFiles'),
      message: t('menu.FileMenu.confirmClearRecentFiles')
    }).then((result) => {
      if (result) {
